...
//                {resolvedSummary?.slice().reverse().map(([name, count], i) => (
//                     <div
//                         key={`resolved-${i}`}
//                         style={{
//                             padding: "10px 12px",
...
const PieAndResolvesAnalysis = ({ data = [] }) => {
    ...
    const insights = Array.isArray(data)
        ? data.filter(
            ticket => ticket?.status === "resolved" || ticket?.status === "assigned"
        )
        : [];
    return (
        <div
        ...
        >
            ...
            >
                ...
                <h6 style={{ fontWeight: 600 }}>Latest 10 Insights</h6>

                {insights?.slice(0, 10).map((ticket, i) => {
                    console.log("ticket - ", i, ticket);

                    const name = ticket?.assignedTo?.name || "Unknown";
                    const resolvedNote = ticket?.notes?.find(n => n?.status === "resolved");
                    const resolvedDate = resolvedNote?.changedOn || null;

                    const assignedNote = ticket?.notes?.find(n => n?.status === "assigned");
                    const assignedDate = assignedNote?.changedOn;
                    if (ticket.status === "resolved") {
                        return (
                            <div
                                key={`resolved-${i}`}
                            ...
                            >
                                <div>
                                    <strong>1</strong> ticket resolved by <strong>{name}</strong>
                                </div>

                                <div style={{ fontSize: "12px", color: "#6c757d" }}>
                                    {resolvedDate ? normalizeDates(resolvedDate) : "-"}
                                </div>
                            </div>
                        );
                    }
                    ...
                    if (ticket.status === "assigned") {
                        return (
                            ...
                            >
                                <div>
                                    <strong>1</strong> ticket assigned to <strong>{name}</strong>
                                </div>

                                <div style={{ fontSize: "12px", color: "#6c757d" }}>
                                    {assignedDate ? normalizeDates(assignedDate) : "-"}
                                </div>
                            </div>
                        );
                    }
